...
if msvcrt:
    ...
    GetConsoleOutputCP = ctypes.WINFUNCTYPE(UINT)(
        ("GetConsoleOutputCP", ctypes.windll.kernel32)
    )
    SetConsoleOutputCP = ctypes.WINFUNCTYPE(BOOL, UINT)(
        ("SetConsoleOutputCP", ctypes.windll.kernel32)
    )
    GetConsoleMode = ctypes.WINFUNCTYPE(
        BOOL, HANDLE, ctypes.POINTER(DWORD), use_last_error=True
    )(("GetConsoleMode", ctypes.windll.kernel32))
    GetNumberOfConsoleInputEvents = ctypes.WINFUNCTYPE(
        BOOL, HANDLE, ctypes.POINTER(DWORD), use_last_error=True
    )(("GetNumberOfConsoleInputEvents", ctypes.windll.kernel32))
    ReadConsoleW = ctypes.WINFUNCTYPE(
        BOOL,
        HANDLE,
        ctypes.c_void_p,
        DWORD,
...
if pythonapi:
    ...
    def getbuffer(b, writable):
        ...
        pythonapi.PyObject_GetBuffer(
            ctypes.py_object(b), ctypes.byref(arr), ctypes.c_int(writable)
        )
        try:
            buf = (ctypes.c_ubyte * arr.len).from_address(arr.buf)
        finally:
            pythonapi.PyBuffer_Release(ctypes.byref(arr))
        return buf
    ...
    while True:
        if line is None:
            while True:  # process preprocessor directives
                counter += 1
                not_in_the_middle_of_any_input = (
                    not in_string
                    and i == j
        ...
        if j < len(line):
            (j, delim) = min(
                map(
                    lambda pair: pair if pair[0] >= 0 else (len(line), pair[1]),
                    map(
                        lambda d: (line.find(d, j), d),
                        in_string or delims if in_string != "--" else "\n",
                    ),
                )
            )
            if i < j:
                concat.append(line[i:j])
                i = j
            if not in_string:
                if j < len(line):
                    j += len(delim)
                    if delim == ";":
                        ...
                        concat.append(
                            line[j : j + len(delim)]
                        )  # ensure delimeter is the same type as the string (it may not be due to implicit conversion)
...
class WindowsConsoleIOMixin(object):
    ...
    def writable(self):
        ...
        return (
            WriteConsoleW(
                self.handle, ctypes.c_void_p(), n, ctypes.byref(n), ctypes.c_void_p()
            )
            != 0
        )
    ...
    def readwcharsinto(self, buf, n):
        ...
        success = ReadConsoleW(
            self.handle, buf, nr, ctypes.byref(nr), ctypes.c_void_p()
        )
...
class WindowsConsoleTextIO(WindowsConsoleIOMixin, io.TextIOBase):
    ...
    def do_read(self, nchars, translate_newlines):
        ...
        empty = os.linesep[:0]
        if nchars is None or nchars < -1:
            nchars = -1
        ncodeunits = (
            nchars if nchars >= 0 else io.DEFAULT_BUFFER_SIZE
        )  # Unit mismatch, but doesn't matter; we'll loop
        buf = None
        istart = 0
        while True:
            iend = (
            ...
            if iend >= 0:
                iend += len(newline) if newline is not None else 0
            if 0 <= iend <= len(self.buffered):
                break
            if buf is None:
                buf = self.getbuf(ncodeunits)
            istart = len(self.buffered)
            chunk = buf[: self.readwcharsinto(buf, ncodeunits)]
            if translate_newlines:
                chunk = chunk.replace(prenewline, empty)
            if chunk.startswith(
                "\x1A"
            ):  # EOF on Windows (Ctrl+Z) at the beginning of a line results in the entire rest of the buffer being discarded
                iend = istart
                break
    ...
    def read(self, nchars=-1):
        return WindowsConsoleTextIO.do_read(self, nchars, None, self.translate)

    def readline(self, nchars=-1):
        return WindowsConsoleTextIO.do_read(self, nchars, self.translate)

    def write(self, text):
        buf = ctypes.create_unicode_buffer(text)
        return self.writewchars(buf, max(len(buf) - 1, 0))
            ...
            try:
                if True or is_output:
                    ...
                    result = io.TextIOWrapper(
                        (io.BufferedWriter if is_output else io.BufferedReader)(
                            WindowsConsoleRawIO(fd)
                        ),
                        "utf-16-le",
                        "strict",
                        line_buffering=True,
...
class NonOwningTextIOWrapper(io.TextIOWrapper):
    def __init__(self, base_textiowrapper, **kwargs):
        ...
        super(NonOwningTextIOWrapper, self).__init__(
            base_textiowrapper.buffer, **kwargs
        )
...
def wrap_unicode_stdio(
    stream, is_writer, encoding
):  # The underlying stream must NOT be used directly until the stream returned by this function is disposed of
...
class StringEscapeParser(object):
    ...
    def __call__(self, s):
        ...
        for match in self.pattern.finditer(s):
            ...
            if m1 is not None:
                result.append(escape_pattern.sub(escape_replacement, m1))
            if m2 is not None:
                result.append(m2)
            if m3 is not None:
                result.append(escape_pattern.sub(escape_replacement, m3))
        return result
...
class StdIOProxy(object):
    ...
    def __init__(self, stdin, stdout, stderr, codec, allow_set_code_page):
        ...
        for stream in streams:
            assert (
                isinstance(stream, io.IOBase)
                or sys.version_info[0] < 3
                and isinstance(stream, file)
                or hasattr(stream, "mode")
            ), "unable to determine stream type"
            assert not isinstance(
                stream, io.RawIOBase
            ), "RAW I/O APIs are different and not supported"
        self.streaminfos = tuple(
            map(
                lambda stream: (
                    stream,
                    isinstance(stream, io.BufferedIOBase)
                    or isinstance(stream, io.RawIOBase)
                    or not isinstance(stream, io.TextIOBase)
    ...
    @property
    def stdin(self):
        return self.streaminfos[0][0]

    @property
    def stdout(self):
        return self.streaminfos[1][0]

    @property
    def stderr(self):
        return self.streaminfos[2][0]
    ...
    def _coerce(self, streaminfo, codec, arg):
        ...
        if (
            ...
            and not isinstance(arg, buffer)
            and not isinstance(arg, unicode)
        ):
            arg = unicode(arg)
    ...
    def _do_write(stream, allow_set_code_page, *args):
        ...
        old_code_page = (
            GetConsoleOutputCP()
            if msvcrt and GetConsoleOutputCP and isatty(stream)
            else None
        )
        if old_code_page == new_code_page:
            old_code_page = None  # Don't change code page if it's already correct...
    ...
    def _readln(self, streaminfo, codec, prompt):
        ...
        if (
            can_call_input_for_stdio(stream) and not can_binary
        ):  # input() can't work with binary data
            result = self._coerce(streaminfo, codec, "")
            try:
                result = input(
                    *(
                        (self._coerce(streaminfo, codec, prompt),)
    ...
    def _writeln(self, streaminfo, codec, *args, **kwargs):
        ...
        for arg in args:
            if first:
                ...
                StdIOProxy._do_write(
                    stream, allow_set_code_page, self._coerce(streaminfo, codec, sep)
                )
            StdIOProxy._do_write(
                stream, allow_set_code_page, self._coerce(streaminfo, codec, arg)
            )
        if end is not None:
            StdIOProxy._do_write(
                stream, allow_set_code_page, self._coerce(streaminfo, codec, end)
            )
        if flush:
            stream.flush()

    def inputln(self, prompt=None):
        return self._readln(self.streaminfos[0], self.codec, prompt)

    def output(self, *args, **kwargs):
        kwargs.setdefault("end", None)
        return self._writeln(self.streaminfos[1], self.codec, *args, **kwargs)

    def outputln(self, *args, **kwargs):
        return self._writeln(self.streaminfos[1], self.codec, *args, **kwargs)

    def error(self, *args, **kwargs):
        kwargs.setdefault("end", None)
        return self._writeln(self.streaminfos[2], self.codec, *args, **kwargs)

    def errorln(self, *args, **kwargs):
        return self._writeln(self.streaminfos[2], self.codec, *args, **kwargs)


class bytes_comparable_with_unicode(
    bytes
):  # For Python 2/3 compatibility, to allow implicit conversion between strings and bytes when it is safe. (Used for strings like literals which we know be safe.)
    codec = codecs.lookup("ascii")  # MUST be a safe encoding

    @classmethod
    def coerce(cls, other, for_output=False):
        return (
            cls.codec.encode(other)[0]
            if not isinstance(other, bytes)
            else bytes_comparable_with_unicode(other)
            if for_output
            else other
        )
    ...
    def __getitem__(self, index):
        return self.coerce(
            super(bytes_comparable_with_unicode, self).__getitem__(index), True
        )

    def __add__(self, other):
        return self.coerce(
            super(bytes_comparable_with_unicode, self).__add__(self.coerce(other)), True
        )
    ...
    def __radd__(self, other):
        return self.coerce(self.coerce(other).__add__(self), True)

    def find(self, other, *args):
        return super(bytes_comparable_with_unicode, self).find(
            self.coerce(other), *args
        )
...
def main(
    program, *args, **kwargs
):  # **kwargs = dict(stdin=file, stdout=file, stderr=file); useful for callers who import this module
    ...
    argparser.add_argument(
        "-version", "--version", action="store_true", help="show SQLite version"
    )
    argparser.add_argument(
        "-batch", "--batch", action="store_true", help="force batch I/O"
    )
    argparser.add_argument(
        "-init", "--init", metavar="FILE", help="read/process named file"
    )
    argparser.add_argument(
        "filename",
        nargs="?",
    ...
    argparser.add_argument(
        ...
        nargs="*",
        metavar="SQL",
        help="SQL commnds to execute after opening database",
    )
    argparser.add_argument(
        "--readline",
        action="store",
        ...
        default="true",
        choices=("true", "false"),
        help="whether to import readline if available (default: %(default)s)",
    )
    argparser.add_argument(
        "--self-test", action="store_true", help="perform a basic self-test"
    )
    argparser.add_argument(
        "--cross-test",
        action="store_true",
        help="perform a basic test against the official executable",
    )
    argparser.add_argument(
        "--unicode-stdio",
        action="store",
        ...
        default="true",
        choices=("true", "false"),
        help="whether to enable Unicode wrapper for standard I/O (default: %(default)s)",
    )
    argparser.add_argument(
        "--console",
        action="store",
        ...
        default="true",
        choices=("true", "false"),
        help="whether to auto-detect and use console window APIs (default: %(default)s)",
    )
    argparser.add_argument(
        "--encoding",
        default=ENCODING,
        help="the default encoding to use (default: %(default)s)",
    )
    (stdin, stdout, stderr) = (
        kwargs.pop("stdin", sys.stdin),
        kwargs.pop("stdout", sys.stdout),
    ...
    allow_set_code_page = (
        sys.version_info[0] < 3 and False
    )  # This is only necessary on Python 2 if we use the default I/O functions instead of bypassing to ReadConsole()/WriteConsole()
    ...
    if (
        not parsed_args.batch
        and isatty(stdin)
        and (parsed_args.readline == "true" or __name__ == "__main__")
        and parsed_args.readline != "false"
    ):
        try:
    ...
    def exec_script(db, filename, ignore_io_errors):
        try:
            with io.open(
                filename, "r", encoding=codec.name
            ) as f:  # Assume .sql files are text -- any binary data inside them should be X'' encoded, not embedded directly
                for command in sql_commands(
                    wrap_bytes_comparable_with_unicode_readline(
...
""".lstrip()
            ...
            if query is not None:
                results = db.cursor.execute(
                    ...
                    else codec.decode(query, "surrogatereplace")[0],
                    query_parameters,
                )
        except (
            RuntimeError,
        ...
        if results is not None:
            for row in results:
                stdio.outputln(
                    *tuple(map(lambda item: item if item is not None else "", row)),
                    sep="|",
                    flush=False
                )
            stdio.output()
    ...
    if db:
            ...
            if index < len(init_sql):
                ...
                if show_prompt:
                    if not_in_the_middle_of_any_input:
                        ...
                        if index == 0:
                            to_write.append(
                                'SQLite version %s (adapter version %s)\nEnter ".help" for usage hints.\n'
        ...
        for command in sql_commands(
            wrap_bytes_comparable_with_unicode_readline(read_stdin)
        ):
...
def cross_test(sqlite_cmdline, codec):
    ...
    if official_output != expected_bytes:
        raise sqlite3.ProgrammingError(
            "expected bytes are wrong: official %s != expected %s"
...
def self_test(codec):
    (query_bytes, expected_bytes) = test_query()
    if not (
        lambda stdin, stdout, stderr: not main(
            sys.argv[0], stdin=stdin, stdout=stdout, stderr=stderr
        )
        and stdout.getvalue() == expected_bytes
    )(io.BytesIO(query_bytes), io.BytesIO(), io.BytesIO()):
        raise sqlite3.ProgrammingError("byte I/O is broken")
    if not (
        lambda stdin, stdout, stderr: not main(
            sys.argv[0], stdin=stdin, stdout=stdout, stderr=stderr
        )
        and stdout.getvalue() == codec.decode(expected_bytes, "surrogateescape")
    )(io.StringIO(query_bytes.decode(ascii)), io.StringIO(), io.StringIO()):
