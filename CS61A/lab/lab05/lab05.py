...
def closer_city(lat, lon, city_a, city_b):
    ...
    dis1=distance(loc,city_a)
    dis2=distance(loc,city_b)
...
def sprout_leaves(t, leaves):
    ...
    return tree(label(t),[sprout_leaves(item,leaves) for item in branches(t)])
...
def add_trees(t1, t2):
    ...
    def recur(t1,t2):
        ...
        for i in range(max(len1,len2)):
            if i<min(len1,len2):
                ret.append(recur(branches(t1)[i],branches(t2)[i]))
            else:
                if i>=len1:
                    ret.append(branches(t2)[i])
                else:
                    ret.append(branches(t1)[i])
        return tree(label(t1)+label(t2),ret)
    return recur(t1,t2)
