...
def replace_leaf(t, find_value, replace_value):
    ...
    if label(t)==find_value and is_leaf(t):
        return tree(replace_value,[replace_leaf(item,find_value,replace_value) for item in branches(t)])
    else:
        return tree(label(t),[replace_leaf(item,find_value,replace_value) for item in branches(t)])
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert not(lower_bound(y)<=0 and upper_bound(y)>=0),"AssertionError"
...
def check_par():
    ...
    r2 = interval(1, 12) # Replace this line!
    return r1, r2
...
def quadratic(x, a, b, c):
    ...
    if extreme_point>lower_bound(x) and extreme_point<upper_bound(x):
        if a>0:
            return interval(f(extreme_point),max(f(lower_bound(x),upper_bound(x))))
        else:
