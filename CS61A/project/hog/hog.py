...
def swine_align(player_score, opponent_score):
    ...
    # END PROBLEM 4a
    GCD=0
    l=list(range(1,min(player_score,opponent_score)+1))
    for i in l[::-1]:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0,score1)<goal:
        if who==0:
            play0_num=strategy0(score0,score1)
            score0+=take_turn(play0_num,score1,dice)
        else:
            play1_num=strategy1(score1,score0)
            score1+=take_turn(play1_num,score0,dice)
        if (who==1 and extra_turn(score1,score0)==False) or (who==0 and extra_turn(score0,score1)==False):
            who=other(who)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    ret=1
    for i in range(1,11):
        roll_test=make_averaged(roll_dice,trials_count)
        t=roll_test(i,dice)
        if t>max:
            max=t
            ret=i
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if free_bacon(opponent_score)>=cutoff or extra_turn(free_bacon(opponent_score)+score,opponent_score):
        return 0
    else:
        return num_rolls  # Replace this statement
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if free_bacon(opponent_score)>=7 or extra_turn(free_bacon(opponent_score)+score,opponent_score):
        return 0
    else:
        return 4
    # END PROBLEM 12
