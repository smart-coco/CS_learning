...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        current_formals=formals
        current_vals=vals
        while current_formals!=nil:
            new_frame.define(current_formals.first,current_vals.first)
            current_formals=current_formals.rest
            current_vals=current_vals.rest
        return new_frame
        # END PROBLEM 10
...
def make_let_frame(bindings, env):
    ...
    while current!=nil:
        ...
        names=Pair(current.first.first,names)
        values=Pair(scheme_eval(current.first.rest.first,env),values)
        current=current.rest
