...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        current=self.place
        distance=0
        while not self.min_range<=distance<=self.max_range or current.bees==[]:
            current=current.entrance
            if current.entrance==None:
                break
            distance+=1
        if current==beehive or current.bees==[] or distance>self.max_range:
            return None
        else:
            return rANTdom_else_none(current.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.is_real==False:
            ...
            while current!=None:
                if current.ant != None and current.ant not in self.record_ant:
                        self.record_ant.append(current.ant) 
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.direction==False and self.place.entrance.name!="Hive":
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        for item in temp:
            Insect.reduce_armor(item,self.damage)
        # END Problem Optional 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while current!=beehive:
            ...
            for item in temp:
                ret[item]=distance
            if current.ant!=None and current!=self.place:
                ret[current.ant]=distance
            current=current.entrance
            if current==self.place:
                break
            distance+=1
        return ret
        # END Problem Optional 5
    ...
    def calculate_damage(self, distance):
        ...
        print("DEBUG:insects_shot:",self.insects_shot)
        print("DEBUG:",damage)
